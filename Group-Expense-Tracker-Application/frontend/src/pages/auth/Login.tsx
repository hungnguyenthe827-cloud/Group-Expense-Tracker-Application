...
export default function Login() {
  ...
  // ✅ CHECK LOCAL LOGIN TRƯỚC
  if (email === MOCK_USER.email && password === MOCK_USER.password) {
    localStorage.setItem("token", "mock-token-123"); // token giả
    ...
    try {
      ...
      if (response.data && response.data.token) {
        ...
        setCurrentUser(response.data.email || email);
        navigate("/dashboard");
      }
    } catch (err: any) {
            ...
            <GoogleLogin
              onSuccess={async (credentialResponse) => {
                try {
                  ...
                  if (response.data && response.data.token) {
                    localStorage.setItem("token", response.data.token);
                    setCurrentUser(response.data.email || email || "google-user");
                    navigate("/dashboard");
                  }
                } catch (err: any) {
